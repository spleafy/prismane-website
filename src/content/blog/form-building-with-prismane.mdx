...
<Note>💡Make sure to provide the same name to the `register` function, as the one that you provided in the `fields` object, otherwise the field will not register correctly!</Note>
...
<Note>💡 As you can see, form submission is unavailable until all of the fields are error-free.</Note>
