Prismane provides a set of global CSS variables that facilitate seamless theme integration into your applications.
With Prismane's global CSS variables, you can achieve consistent theming across your entire application,
making it effortless to create a cohesive and personalized user experience.
...
<Warning>
    If no theme object is passed to the provider, the base theme will be used instead.
</Warning>
...
This allows you to be flexible in their usage.

## Inside CSS Stylesheet

If you want to use the Prismane's CSS variales inside your css file, you can use the `var` function with the name of
the css variable.
...
.some-component-class {
  background-color: var(--prismane-colors-primary-100);
}
...
You can use the CSS variables inside the styling of the components as well.
```tsx
...
const Page = () => {
  return (
    <Box
      style={{
        // Use the Prismane's CSS variable
...
This is a list of all CSS variables that Prismane adds to your application:
...
| `--prismane-border-radius-xs`   | theme.borderRadius.xs     |
| `--prismane-border-radius-sm`   | theme.borderRadius.sm     |
| `--prismane-border-radius-base` | theme.borderRadius.base   |
| `--prismane-border-radius-md`   | theme.borderRadius.md     |
| `--prismane-border-radius-lg`   | theme.borderRadius.lg     |
| `--prismane-border-radius-xl`   | theme.borderRadius.xl     |
| `--prismane-border-radius-2xl`  | theme.borderRadius["2xl"] |
| `--prismane-font-family`        | theme.fontFamily          |
